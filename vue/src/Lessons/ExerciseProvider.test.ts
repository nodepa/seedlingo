...
beforeEach(() => {
  lesson = {
    lessonId: '9b56ad90-9bee-48b7-baf5-e8372e2001b6',
    lessonIndex: 1,
    wordsCount: 4,
    phrasesCount: 0,
    items: [
...
describe('ExerciseProvider', () => {
  describe('.getExerciseFromLesson()', () => {
    it('correctly returns exercises', () => {
      ...
      const { pickRandomIndexLessThan } = ExerciseProvider;
      ExerciseProvider.pickRandomIndexLessThan = jest.fn(() => 3);
      ...
      ExerciseProvider.pickRandomIndexLessThan = pickRandomIndexLessThan;
    });
  });
  ...
  describe('.generateMultipleChoicePhraseExercise()', () => {
    it('correctly returns exercises', () => {
      lesson = {
        lessonId: '17df7241-eb9d-4bea-904e-d51823d8fc85',
        lessonIndex: 4,
        wordsCount: 2,
        phrasesCount: 2,
        items: [
          {
            ...
            word: '姥姥',
            audioName: 'placeholder-audio.mp3',
            pictureName: '',
            videoName: '',
          ...
          },
          {
            ...
            word: '',
            phrase: '妈妈的妈妈',
            phraseInterpretationId: 'd63f08dd-f8e6-4b47-940b-8d9d939034a6',
            audioName: 'placeholder-audio.mp3',
            pictureName: '',
            videoName: '',
            symbolName: ['mdiCellphoneWireless'],
          ...
          },
          {
            ...
            word: '',
            phrase: '妈妈的爸爸',
            phraseInterpretationId: '6c0f975c-4364-40c7-ae72-cfc2145da7e3',
            audioName: 'placeholder-audio.mp3',
            pictureName: '',
            videoName: '',
            symbolName: ['mdiCellphoneWireless'],
      ...
      const multipleChoiceExercise = ExerciseProvider.generateMultipleChoicePhraseExercise(
        lesson,
      ...
      expect(multipleChoiceExercise.exerciseItems.phraseToMatch).toBeDefined();
      if (multipleChoiceExercise.exerciseItems.phraseToMatch) {
        expect(
          multipleChoiceExercise.exerciseItems.phraseToMatch.length,
        ).toBeGreaterThanOrEqual(3);
      }

      // assert correct option is marked 'correct'
      if (
        multipleChoiceExercise.exerciseItems.phraseToMatch ===
        lesson.items[1].phrase
      ) {
  ...
  describe('.pickRandomIndexLessThan', () => {
    it('is covered', () => {
      expect(ExerciseProvider.pickRandomIndexLessThan(0)).toBe(0);
      expect(ExerciseProvider.pickRandomIndexLessThan(1) === 0).toBe(true);
      expect(ExerciseProvider.pickRandomIndexLessThan(2) < 2).toBe(true);
    });
  });
