  ...
  <UModal :title="props.title" :description="props.description" close
    ...
    <template #body>
      ...
      <div class="flex justify-end space-x-2">
        ...
        <UButton color="gray" @click="showMediaModal = false">
          Cancel
        </UButton>
      </div>
    </template>
...
const filesAdded = (uploadFiles: Array<File>) => {
  ...
  uploadFiles.forEach((file) => {
    ...
    fileReader.onload = async (e) => {
      ...
      if (fileReader.result) {
        const fileMeta = { name: file.name, state: { fileReader, progress: 0 } };
        mediaFiles.push(fileMeta);
        try {
          const result = await uploadData({
            path: `word-pictures/${user.userId}/${file.name}`,
