      ...
      >
        ...
        >
          ...
          <div class="relative w-40">
            ...
            <div
              v-if="!file.data"
              class="w-40 h-28 rounded-md bg-gray-500 animate-pulse"
            ></div>
            <NuxtImg
              v-else-if="mediaType === 'pictures'"
              :src="file.data"
              :alt="file.name"
...
const props = withDefaults(
  ...
  { mediaType: 'pictures', title: '', description: '', selected: '' },
);
...
const fetchStoredMedia = async () => {
  ...
  try {
    ...
    [...pictureList, ...audioList].forEach((file) => {
      const mediaType = file.path.startsWith('word-audio/')
        ? 'audio'
        : 'pictures';
