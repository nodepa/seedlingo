<template>
  <UForm :schema="FileSchema" :state="state" class="space-y-4">
...
const onFileChange = (event: Event) => {
  ...
  if (files) {
    ...
    console.log('file validation output:', validation.output);
    console.log('output files:');
    (validation.output as FileArrayType).files.forEach((file: File) => {
    ...
    if (!validation.success) {
      validation.issues.forEach((issue, index) => {
        ...
        console.log('issue:', (issue.input as File)?.name, issue.type, issue.received, issue.message);
      });
    } else {
