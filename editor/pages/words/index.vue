  ...
  <UContainer>
    <UCard :ui="{ body: 'sm:p-0' }">
      ...
      <UTable :data="wordsState" :columns="columns"
        ...
        <template #word-cell="{ row }" class="w-10">
          ...
          <textarea type="text"
            ...
            @blur="(event) => commitCell(row, 'description')"
            @keydown="($event) => $event.ctrlKey && $event.key === 'Enter' && commitCell(row, 'description')"
        ...
        <template #isPunctuation-cell="{ row }">
          <USwitch v-model="row.original.isPunctuation" color="primary"
            @change="($event) => commitCell(row, 'isPunctuation')"
      ...
      <template #footer>
        ...
        <input value="test"
          class="p-2 field-sizing-content text-(--ui-secondary) bg-(--ui-neutral)" />
...
const viewClick = (row: Row<DynamicWord>) => {
...
watch(() => wordsState.value, (updatedWords) => {
...
const save = async (word: DynamicWord) => {
  ...
  if (word.id) {
    ...
    if (errors || !existingWord) {
      ...
      console.log('word saved to server:', updatedWord);
    }
  } else {
