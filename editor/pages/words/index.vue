    ...
    <UCard :ui="{ body: 'sm:p-0' }">
      ...
      >
        <template #word-cell="{ row }">
          <div class="h-28 flex flex-col gap-1 justify-start">
            <UInput
              ...
              @keydown="
                ($event: KeyboardEvent) =>
                  $event.key === 'Enter' && commitCell(row, 'word')
              "
            />
            <UTextarea
        ...
        <template #picture-cell="{ row }">
          <MediaModal
            v-model:selected="row.original.picture"
        ...
        <template #audio-cell="{ row }">
          <MediaModal
            v-model:selected="row.original.audio"
            :title="row.original.audio ? 'Update audio' : 'Add audio'"
            description="Upload, organize and link media to words."
            media-type="audio"
        ...
        <template #isPunctuation-cell="{ row }">
          <USwitch
            v-model="row.original.isPunctuation"
            color="primary"
        ...
        <template #tags-cell="{ row }">
          <div class="flex flex-wrap gap-1 min-w-32 py-1">
            <UBadge
              v-for="tag in getWordTags(row.original.id)"
              :key="tag.id"
              color="primary"
              variant="subtle"
            ...
            <UPopover
              ...
              @update:open="
                (v: boolean) =>
                  (openTagPopoverWordId = v ? row.original.id : null)
              "
            >
      ...
      <template #footer>
        <div class="my-4 flex justify-center">
          <WordForm
            is-add-mode
            :available-tags="tagsState"
            @update-word="createWord"
          />
        </div>
        <p class="text-center">
          Showing {{ filteredWords.length }}
          <template v-if="selectedFilterTags.length > 0"
            >of {{ wordsState.length }}</template
          >
          words
        </p>
      </template>
...
const getWordTags = (wordId: string): TagSchema[] => {
  ...
  return tags.filter(
    (t) => !pendingRemovals.value.includes(`${wordId}:${t.id}`),
  );
};

const availableTagsForWord = (wordId: string): TagSchema[] => {
  const assignedIds = new Set(
    (wordTagMap.value.get(wordId) ?? ([] as TagSchema[])).map(
      (t: TagSchema) => t.id,
    ),
  );
...
const toggleFilter = (tagId: string) => {
  ...
  if (idx === -1) {
    ...
    selectedFilterTags.value = selectedFilterTags.value.filter(
      (id) => id !== tagId,
    );
  }
};
...
const addTagToWord = async (wordId: string, tagId: string) => {
  ...
  const { data: newWordTag, errors } = await client.models.WordTag.create({
    wordId,
    tagId,
  });
  ...
  if (errors || !newWordTag) {
    ...
    toast.add({
      title: 'Error',
      description: 'Failed to add tag to word',
      color: 'error',
    });
    ...
    if (
      openTagPopoverWordId.value === wordId &&
      availableTagsForWord(wordId).length === 0
    ) {
...
const removeTagFromWord = async (wordId: string, tagId: string) => {
  ...
  const wordTag = wordTagsState.value.find(
    (wt) => wt.wordId === wordId && wt.tagId === tagId,
  );
  if (!wordTag) {
  ...
  if (errors) {
    ...
    toast.add({
      title: 'Error',
      description: 'Failed to remove tag from word',
      color: 'error',
    });
  } else {
    wordTagsState.value = wordTagsState.value.filter(
      (wt) => wt.id !== wordTag.id,
    );
  }
};
...
const save = async (word: DynamicWord) => {
  word.isWaiting = true;
  if (word.id) {
    const { data: existingWord, errors } = await client.models.Word.get({
      id: word.id,
    });
    ...
    const { errors } = await client.models.Word.create(newWord);
    if (errors) {
      console.error(errors);
      toast.add({
        title: 'Error',
...
const createWord = async (wordData: DynamicWord & { tagIds?: string[] }) => {
  if (wordData.id) {
    ...
    const { data: createdWord, errors } =
      await client.models.Word.create(newWord);
    ...
    if (wordData.tagIds?.length) {
      await Promise.all(
        wordData.tagIds!.map((tagId: string) =>
          client.models.WordTag.create({ wordId: createdWord.id, tagId }),
        ),
      );
    }
  }
