        ...
        <li v-for="unit in units" :key="'unit-id-' + unit.id" class="flex w-xs">
          <UCard class="w-full grid">
            ...
            <div v-else class="h-full">
              <UForm :schema="v.safeParser(UnitSchema)" :state="unit"
...
onBeforeMount(() => {
  createSub = client.models.Unit.onCreate().subscribe({
    next: (data) => console.log('db-reports-created:', data),
    error: (error) => console.warn(error),
  });
  updateSub = client.models.Unit.onUpdate().subscribe({
    next: (data) => console.log('db-reports-updated:', data),
    error: (error) => console.warn(error),
  });
  deleteSub = client.models.Unit.onDelete().subscribe({
